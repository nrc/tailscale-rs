...
impl<TableStorage: schema::GeneratedStorage> KvStore<TableStorage> {
    ...
    pub fn begin_transaction(&self, owner: Owner) -> TableStorage::Transaction<'_> {
        ...
        TableStorage::make_txn(Transaction {
            guard: Some(guard),
            owner,
            id,
    ...
    pub fn try_begin_transaction(&self, owner: Owner) -> Option<TableStorage::Transaction<'_>> {
        ...
        Some(TableStorage::make_txn(Transaction {
            guard: Some(guard),
            owner,
            id,
            _not_send_or_sync: PhantomData,
...
> {
    /// Invariant: the transaction outlives `self` and `'a`.
    txn: *mut Transaction<'a, TableStorage>,
    desc: PhantomData<S>,
}
...
{
    /// SAFETY: the caller must ensure that the target of `txn` will outlive `'a`.
    #[doc(hidden)]
    pub unsafe fn new(txn: *mut Transaction<'a, TableStorage>) -> Self {
    ...
    {
        <&mut Self as SingletonOpsMut<_>>::with_mut::<S, T>(self, f, self.owner())
    }

    /// Insert a single value into the store.
    pub fn insert(&mut self, value: S::Value) {
        <&mut Self as SingletonOpsMut<_>>::insert::<S>(self, value, self.owner())
    }

    /// Remove a single value from the store.
    pub fn remove(&mut self) {
        <&mut Self as SingletonOpsMut<_>>::remove::<S>(self, self.owner())
    }
}
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> Ops<TableStorage> for &'a SingletonTransaction<'guard, TableStorage, S>
{
...
impl<
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> SingletonOps<TableStorage> for &SingletonTransaction<'_, TableStorage, S>
{
}
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> OpsMut<TableStorage> for &'a mut SingletonTransaction<'guard, TableStorage, S>
{
...
impl<
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> SingletonOpsMut<TableStorage> for &mut SingletonTransaction<'_, TableStorage, S>
{
}
...
> {
    /// Invariant: the transaction outlives `self` and `'a`.
    txn: *const RoTransaction<'a, TableStorage>,
    desc: PhantomData<S>,
}
...
> RoSingletonTransaction<'a, TableStorage, S>
{
    /// SAFETY: the caller must ensure that the target of `txn` will outlive `'a`.
    #[doc(hidden)]
    pub unsafe fn new(txn: *const RoTransaction<'a, TableStorage>) -> Self {
    ...
    fn owner(&self) -> Owner {
        // SAFETY: safe since `self.txn` must be live since `self` is (by it's field invariant), and `owner`
        // has `'static` lifetime.
        unsafe { (*self.txn).owner }
    }
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> Ops<TableStorage> for &'a RoSingletonTransaction<'guard, TableStorage, S>
{
    type ReadLock = &'a RwLockReadGuard<'guard, Storage<TableStorage>>;
...
impl<
    TableStorage: schema::GeneratedStorage + 'static,
    S: schema::SingletonDesc<Storage = TableStorage>,
> SingletonOps<TableStorage> for &RoSingletonTransaction<'_, TableStorage, S>
{
}
    ...
    TableTransaction<'a, TableStorage, D>
{
    ...
    /// The transaction this is a view of.
    ///
    /// Used to reach the transaction from types which have no view of their own in the generated
    /// transaction (i.e., indexes).
    pub(crate) fn store_txn(&mut self) -> &mut Transaction<'a, TableStorage> {
        // SAFETY: safe since `self.txn` must be live since `self` is (by it's field invariant).
        unsafe { &mut *self.txn }
    }
    ...
    pub fn clear(&mut self) {
        <&mut Self as TabularOpsMut<_>>::clear(self, self.owner())
    }
    ...
    where
        ...
        Q: ?Sized + Hash + Eq,
    {
        <&Self as TabularOps<_>>::get::<Q>(self, key, self.owner())
    }
    ...
    where
        ...
        Q: ?Sized + Hash + Eq,
    {
        <&Self as TabularOps<_>>::with::<Q, T>(self, key, f, self.owner())
    }

    /// Insert a value into the table.
    pub fn insert(&mut self, key: D::Key, value: D::Value) {
        <&mut Self as TabularOpsMut<_>>::insert(self, key, value, self.owner())
    }
    ...
    {
        <&mut Self as TabularOpsMut<_>>::with_mut(self, key, f, self.owner())
    }
    ...
    where
        ...
        Q: ?Sized + Hash + Eq + ToOwned<Owned = D::Key>,
    {
        <&mut Self as TabularOpsMut<_>>::remove(self, key, self.owner())
    }
...
> TableTransaction<'a, TableStorage, D>
{
    ...
    /// field is an [`IndexTransaction`](crate::IndexTransaction) which gives access to the table
    /// using that index's key, as part of this transaction.
    ///
    /// # Example:
    ///
    /// ```rust,ignore
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    D: TableDesc<Storage = TableStorage>,
> Ops<D::Storage> for &'table TableTransaction<'guard, TableStorage, D>
{
    type ReadLock = &'table RwLockWriteGuard<'guard, Storage<D::Storage>>;

    fn read_lock(self) -> Self::ReadLock {
        // SAFETY: safe since `self.txn` must be live since `self` is (by it's field invariant),
        // and the returned lock is a reference lifetime with lifetime `'table`.
        unsafe { (*self.txn).guard.as_ref().unwrap() }
    }
}
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    D: TableDesc<Storage = TableStorage>,
> OpsMut<D::Storage> for &'table mut TableTransaction<'guard, TableStorage, D>
{
...
impl<'guard, TableStorage: schema::GeneratedStorage + 'static, D: TableDesc<Storage = TableStorage>>
    TabularOps<D::Storage> for &TableTransaction<'guard, TableStorage, D>
{
    type TableDesc = D;
}

impl<'guard, TableStorage: schema::GeneratedStorage + 'static, D: TableDesc<Storage = TableStorage>>
    TabularOpsMut<D::Storage> for &mut TableTransaction<'guard, TableStorage, D>
{
    type TableDesc = D;
}
...
{
    /// SAFETY: the caller must ensure that the target of `txn` will outlive `'guard`.
    #[doc(hidden)]
    pub unsafe fn new(txn: *const RoTransaction<'guard, TableStorage>) -> Self {
        RoTableTransaction {
            txn,
            desc: PhantomData,
        }
...
impl<
    ...
    TableStorage: schema::GeneratedStorage + 'static,
    D: TableDesc<Storage = TableStorage>,
> Ops<D::Storage> for &'table RoTableTransaction<'guard, TableStorage, D>
{
    type ReadLock = &'table RwLockReadGuard<'guard, Storage<D::Storage>>;
...
impl<'guard, TableStorage: schema::GeneratedStorage + 'static, D: TableDesc<Storage = TableStorage>>
    TabularOps<D::Storage> for &RoTableTransaction<'guard, TableStorage, D>
{
    type TableDesc = D;
}
